 ...
 */
//TODO implement methods with lodash

var kutral;
kutral = function ($firebaseObject, $firebaseArray, $q, aggregationKeys, aggregation, find, parsers) {
  ...
  var determineTypeOfDataRef = function(sync) {
    ...
    var self = this;
      ...
      self.queue.add(function(self) {
        if(query) {
          ...
          dataRef = determineTypeOfDataRef(sync);

          var useIndex;

          if(query.hasOwnProperty("$id")) {
            if(sync) {
              var dataRef = (asArray) ? $firebaseArray(self.ref.child(query.$id)) : $firebaseObject(self.ref.child(query.$id));
            ...
            var asArray = self.queue._methods.some(function(method) {return (method.name == "asArray") } );
            var dataRef = (asArray) ? $firebaseArray(self.ref) : $firebaseObject(self.ref);
            ...
            if(queryKeys.length == 1 && (queryKeys[0] == "$or" || queryKeys[0] == "$and")) {
              ...
              if(useIndex) {
                var asArray = self.queue._methods.some(function(method) {return (method.name == "asArray") } );
                var dataRef = (asArray) ? $firebaseArray(self.ref.orderByChild(useIndex).equalTo(query[useIndex])) : $firebaseObject(self.ref.orderByChild(useIndex).equalTo(query[useIndex]));
  ...
  fireQuery.prototype.populate = function(params, callback) {
  ...
  fireQuery.prototype.limit = function(limitNumber, callback) {
    ...
    self.queue.add(function(self) {
      if(self.data && limitNumber) {
        self.data = self.data.slice(0,limitNumber);
      }
